...
import { listProductAnalysis, analyzeProductRequests, getProductInsights } from "../api";
...
export function ProductRequestsPage() {
  ...
  function runAnalysis() {
    ...
    analyzeProductRequests({})
      .then((data) => {
        ...
        return getProductInsights().then((fresh) => {
      ...
      .catch((err: Error) => setAnalyzeError(err?.message || "Analysis failed"))
  ...
  const productRequests = useMemo(() =>
    productInsights.flatMap((r) =>
      (r.items || []).map((item, i) => ({
        ...
        summaryBulletGist: r.summaryBulletGist,
      }))
    ),
    [productInsights]
  );
  ...
  const groupedByCall = useMemo(() => {
    ...
    for (const r of filtered) {
      if (!map.has(r.callId)) {
        map.set(r.callId, { callId: r.callId, callTitle: r.callTitle, aeName: r.aeName, date: r.date, transcriptUrl: r.transcriptUrl, summaryBulletGist: r.summaryBulletGist, items: [] });
      }
  ...
  return (
    <div className="p-8 space-y-6">
      {showAnalyzingOverlay && (
        <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-app-base/80 backdrop-blur-sm">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <div className="flex items-center gap-2 bg-app-card border border-[#1e1e2e] rounded-xl px-4 py-2.5 flex-1 min-w-[240px]">
        ...
        <div className="relative" ref={dropdownRef}>
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2.5 rounded-xl border transition-colors cursor-pointer ${
              selectedOrganizer !== "all"
                ...
                : "bg-app-card border-[#1e1e2e] text-[#8888a0] hover:border-[#2e2e3e]"
            }`}
          ...
          {dropdownOpen && (
            <div className="absolute right-0 top-full mt-1.5 w-72 bg-app-card border border-[#1e1e2e] rounded-xl shadow-xl shadow-black/40 z-50 overflow-hidden">
              <div className="max-h-72 overflow-y-auto">
                <button
                  ...
                  className={`w-full flex items-center justify-between px-4 py-2.5 text-left transition-colors cursor-pointer ${
                    selectedOrganizer === "all" ? "bg-[#ec5d25]/10 text-[#ec5d25]" : "text-[#c0c0d0] hover:bg-app-elevated"
                  }`}
                ...
                {organizers.map((org) => (
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-4 py-2.5 text-left transition-colors cursor-pointer ${
                      selectedOrganizer === org ? "bg-[#ec5d25]/10 text-[#ec5d25]" : "text-[#c0c0d0] hover:bg-app-elevated"
                    }`}
                    style={{ fontSize: "0.82rem" }}
                  >
      ...
      {groupedByCall.length > 0 ? (
        <div className="space-y-4">
          {groupedByCall.map((group) => (
            <div
              key={group.callId}
              className="bg-app-card border border-[#1e1e2e] rounded-xl overflow-hidden hover:border-[#2a2a3a] transition-colors"
            >
              <div className="flex items-center justify-between px-5 py-3.5 border-b border-[#1e1e2e] bg-app-surface">
              ...
              <div className="px-5 py-4 space-y-4">
                {group.summaryBulletGist && (
                  <div className="bg-app-surface border border-[#1e1e2e] rounded-lg px-4 py-3">
                    <p className="text-[#555568] mb-2" style={{ fontSize: "0.7rem", fontWeight: 600, letterSpacing: "0.05em", textTransform: "uppercase" }}>Call Summary</p>
                    <ul className="space-y-1">
                      {group.summaryBulletGist.split("\n").filter(Boolean).map((line, i) => (
                        <li key={i} className="flex items-start gap-2">
                          <span className="w-1 h-1 rounded-full bg-[#555568] mt-2 shrink-0" />
                          <span className="text-[#8888a0] leading-relaxed" style={{ fontSize: "0.8rem" }}>{line.replace(/^[-•*]\s*/, "")}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
        ...
        <div className="bg-app-card border border-[#1e1e2e] rounded-xl p-12 text-center">
